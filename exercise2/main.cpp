...
public:
  Salesman &salesman(uint64_t sales_id)
  {
    return this->salesmen[sales_id - 1];
  }
  ...
  void print()
  {
    ...
    for (uint64_t sales_id = 1; sales_id <= SALES; ++sales_id)
    {
      ...
      {
        cout << (double)this->salesman(sales_id).product(product_id).value() / 100.0 << '\t';
      }
