...
void main() {
  ...
  for (;;) {
    ...
    if (!vehicle && action_depend_on_vehicle(action)) {
      std::cout
          << "In order to use this command you need to create a vehicle ~grrr~"
          << std::endl;
      continue;
    }
    ...
    switch (action) {
      case Action_create_car: {
        ...
        vehicle =
            new CarInh(opts.weight, opts.speed, opts.milage, opts.burning);
        break;
      }
      ...
      case Action_create_bus: {
        ...
        vehicle =
            new BusInh(opts.weight, opts.speed, opts.milage, opts.burning);
        break;
      }
